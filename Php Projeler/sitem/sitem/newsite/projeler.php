...
<?php      include 'ayar.php';
include 'func.php';
          $veri=$db->query("select * from projeler where id <=12 " , PDO::FETCH_ASSOC);
  echo "<div class='işler'>"; 
      
        
    

          foreach ($veri as $row)
            echo  '<div data-wow-delay="0.1s" class="işlerr wow bounceInLeft">'.'<a href="projetikla.php?link='.$row["link"].'" style="text-decoration:none;color:black;" >
          <h4>'.$row["baslik"].'</h4></a>'.'<div style="display:flex;flex-direction:row;">'.'<p> <img loading="lazy" src="img/'.$row["projeicon"].'"  width="64" height="64"></p>'.'<p style="margin-left:10px;margin-top:30px;">'.kisalt($row["aciklama"],20).'</p></div></div>';
